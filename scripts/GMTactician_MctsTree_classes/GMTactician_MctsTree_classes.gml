...
function MctsTree(_state) constructor {
	...
	///@func reroot(moves)
	///@param moves Array of moves to make next
	...
	///@param node The node to pick random children from
	...
	///@func expandShuffled(state)
	///@param state The state to expand children moves
	...
	static expandShuffled = function(_state) {
		var _moves = _state.getMoves();
	...
	static playNthBest = function(_state) {
		var _moves = _state.getMoves();
		var _movesN = array_length(_moves);
		var _tempState = _state.clone();
...
///@param move The Move this node represents
///@param lastPlayer The preceding player
